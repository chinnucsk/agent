...
cputask(Dn, Ts, Output) ->
	...
	CpuData = [{cpu1min, list_to_float(Load1)}, 
	...
	TaskMetric = #metric{name='opengoss.localtask',
						  from="agent",
						  dn=Dn,
						  timestamp=Ts, 
						  data=[{taskTotal, TaskTotal}]},
    {CpuInfo, CpuMetric, TaskMetric}.
...
swapmem(Dn, Ts, Output) ->
	...
	MemData = [{memTotal, MemTotal},
			   ...
			   {swapFree, SwapFree},
			   {swapUsage, SwapUsage}],
	MemMetric = #metric{name='opengoss.localmem',
						from="agent",
						dn=Dn, 
						timestamp=Ts,
						data=MemData},
    {MemInfo, SwapInfo, MemMetric}.
...
parse(Dn, Ts, [L1,L2,L3,L4|Lines], InfoAcc, DatalogAcc) ->
	...
	DiskData = [{diskTotal, Total},
			    ...
			    {diskFree, Free},
				{diskUsage, Usage}],
    DiskMetric = #metric{name='opengoss.localdisk',
					  from="agent",
					  dn=DiskDn,
					  timestamp=Ts,
					  data=DiskData},
    parse(Dn, Ts, Lines, [Info|InfoAcc], [DiskMetric|DatalogAcc]).
